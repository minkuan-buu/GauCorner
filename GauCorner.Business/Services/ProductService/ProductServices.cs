...
namespace GauCorner.Business.Services.ProductServices
{
    public class ProductServices : IProductServices
    {
        ...
        public async Task<ResultModel<MessageResultModel>> UpdateProduct(Guid productId, ProductDto productModel, string Token)
        {
            var userId = Guid.Parse(Authentication.DecodeToken(Token, "userid"));

            var product = await _productRepositories.GetSingle(x => x.Id.Equals(productId) && x.CreatedBy == userId);
            if (product == null)
            {
                return new ResultModel<MessageResultModel>
                {
            ...
            product.Name = TextConvert.ConvertToUnicodeEscape(productModel.Name);
            product.Description = TextConvert.ConvertToUnicodeEscape(productModel.Description);
            product.CategoryId = productModel.CategoryId;

            // Xóa dữ liệu cũ
            await _variantAttributeValueRepositories.DeleteByProductId(productId);
            await _attributeValueRepositories.DeleteByProductId(productId);
            await _productAttributeRepositories.DeleteByProductId(productId);
            await _productVariantRepositories.DeleteByProductId(productId);
            await _productAttachmentRepositories.DeleteByProductId(productId);

            // Tạo dữ liệu mới như trong CreateProduct
            var productAttributes = new List<ProductAttribute>();
            var attributeValues = new List<AttributeValue>();
            var attributeIdMap = new List<(int attrIndex, Guid attrId)>();
